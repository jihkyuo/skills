---
name: socratic
description: 모호한 재료(회의록·긴 문서·정책·스펙)를 문답으로 파고들어 구조화된 이해와 구멍 목록으로 바꾸는 방식. "이해시켜줘"·"문답으로 파자"·"소크라틱" 같은 명시 요청이면 바로 시작한다. 재료를 깊이 이해하려는 신호만 보이면, 상대 시간이 상당히 드는 방식(수십 라운드)임을 알리고 한 번 물은 뒤 시작한다. Use when asked to "walk me through this", "quiz me on this doc", "socratic", or "help me really understand this spec / these meeting notes"; explicit requests start at once, softer signals get one heads-up (tens of rounds) before starting. Not for summaries, translations, or lookups of a single fact.
---
...
1. **재료 선행 흡수 — 흡수 선언표를 내기 전에는 첫 질문을 하지 않는다.**
   ...
   *"전부 읽는다"* 는 판정 불가능한 문장이라 그대로 두면 반드시 안 지켜진다. 지켰는지가 **표 하나로 보이게** 만든다. 첫 라운드 맨 앞에 이 표를 내고, `상태`가 전부 ✅가 되기 전에는 어떤 설명도 판정도 하지 않는다.
   ...
   - **줄 번호가 없는 재료(위키·웹 페이지·티켓·채팅 스레드)는 절·헤딩·메시지 단위로 적는다.** 단위가 무엇이든 «전체 크기»와 «읽은 범위»가 같은 단위로 나란히 보여야 한다 — 그래야 미완이 눈에 띈다.
