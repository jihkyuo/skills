...
`~/.claude/skills/handoff/SKILL.md`(mattpocock)를 **읽고 그 절차를 따른다** (`disable-model-invocation`은 Skill 도구 호출만 막을 뿐 파일 읽기는 막지 않는다). 사용자가 밝힌 다음 세션의 목적을 그 절차의 인자로 삼아 문서를 재단한다.

파일이 없으면 이 폴백으로 직접 작성한다 — OS 임시 디렉토리에, 다음 슬롯을 채워서:
- 목표·현재 위치·다음 단계 / **기각한 접근과 기각 사유** / 사용자의 구두 제약 / suggested skills(새 에이전트가 호출할 스킬)
- 산출물(스펙·커밋·diff)은 내용 복제 없이 경로 참조만
- **비밀값·PII는 삭제(redact)** — 특히 외부 모델(codex 등)로 넘길 때
