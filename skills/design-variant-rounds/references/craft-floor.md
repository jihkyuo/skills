<!-- 출처: pbakaus/impeccable@5a7e283 plugin/skills/impeccable/reference/craft-floor.md · Apache-2.0 · verbatim. 우리 예외는 파일 끝 «계약 우선 예외» 절. -->
...
## 계약 우선 예외 (design-variant-rounds)

이 바닥선은 «브리프가 이긴다»를 스스로 명시한다. 씨드 계약이 요구하는 요소는 위 Refuse 목록에 있어도 예외다. 예: 제품 AI 축의 그라데이션 마크(«gradient-text» 검출은 무시 규칙에 등록), 형식 칩·배지처럼 디자인 시스템이 이미 가진 부품. 예외는 씨드 §계약에 한 줄씩 적고, `scripts/detect.mjs` 의 무시 목록에도 같은 줄을 넣는다.
